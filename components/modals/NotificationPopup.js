...
<div className="bg-white w-[588px] h-[670px] rounded-[12px] p-6 flex flex-col shadow-lg">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-4 mb-6">
          <button className="bg-[#5D5FEF] text-white px-4 py-1.5 rounded-md text-sm font-medium">All</button>
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <h4 className="font-semibold text-sm text-black">
                  {item.title}
                  {item.tag && (
                    <span className="block text-xs text-[#4F4F4F]">{item.tag}</span>
                  )}
                </h4>
              ...
              <p className="text-xs text-[#4F4F4F] mt-1 leading-snug">{item.description}</p>
            </div>
          </div>
        ))}
      </div>
