...
// File: SubscriptionUserList.js (or wherever this component is)
...
// <<< FIX 1: Hum yahan par central data file se data import kar rahe hain
import { featureSubscriptionUsers } from "@/lib/subscription-data";

const SubscriptionUserList = () => {
  const [openMenu, setOpenMenu] = useState(null);

  // <<< FIX 2: Humne yahan se local 'subscriptionUsers' array ko DELETE kar diya hai
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden mt-4">
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody className="bg-white">
              {/* <<< FIX 3: Hum ab 'featureSubscriptionUsers' ko map kar rahe hain */}
              {featureSubscriptionUsers.map((user, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {user.subscriptionPlan} {/* Changed from user.plan */}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {user.subscriptionStartDate} {/* Changed from user.startDate */}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {user.subscriptionExpiryDate} {/* Changed from user.expiryDate */}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span
                      className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
